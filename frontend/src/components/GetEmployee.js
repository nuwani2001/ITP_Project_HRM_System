...
function GetEmployee() {
    ...
    return ( 
        <><a style={{ marginRight: "0px", marginLeft: "25px", marginTop: "10px" }} href="/" className="btn btn-secondary">Back <i class="fa fa-reply"/></a>
            <button style={{ marginLeft: "75%", marginRight: "2%" }} className="btn btn-success" onClick={() => {
                window.location.replace(`http://localhost:3000/employees/update/${id}`);
            }}>Update <i class="fa fa-pencil"/> </button>
            <button style={{ marginRight: "1%"}} className="btn btn-danger" onClick={() => {
                window.location.replace(`http://localhost:3000/employees/delete/${id}`);
